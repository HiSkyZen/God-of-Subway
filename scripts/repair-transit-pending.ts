...
const DB_PATH = resolve(ROOT, Bun.env.TRANSIT_DB_PATH?.trim() || "data/transit.deploy-candidate.sqlite");
const MARKER_PATH = resolve(ROOT, Bun.env.TRANSIT_FAILURE_MARKER?.trim() || "data/transit-refresh-failure.json");
const REPAIR_CONCURRENCY = Math.max(
  1,
  Math.min(16, Number(Bun.env.TRANSIT_DEPLOY_REPAIR_CONCURRENCY ?? 8) || 8),
);
...
function applyGroup(db: Database, group: Event[]): void {
  ...
  const before = trip
    ? existingEvents(db, Number(trip.trip_id), group[0].sourceId, day, line, trainNo)
      .filter((event) => !repairedStationIds.has(event.stationId))
    : [];
  const combined = orderedDeduped([...before, ...group]);
  if (combined.length < 2) return;

  const rawKind = group.find((event) => event.rawKind)?.rawKind ?? "";
  const kind = trip?.service_kind || serviceKind(rawKind);
  if (line === "공항철도" && (kind === "direct" || (!trip && combined.length <= 4))) return;
  ...
  if (!tripId) {
    ...
    db.query(`
      UPDATE trip
      SET origin_station_id=?,destination_station_id=?,direction=?
      WHERE trip_id=?
...
if (!(await Bun.file(DB_PATH).exists())) throw new Error(`partial live transit SQLite is missing: ${DB_PATH}`);
...
async function fetchUnit(unit: FailureUnit): Promise<FetchResult> {
  ...
  const station = db.query(`
    SELECT ss.station_id,s.canonical_name,ss.sequence_hint
...
try {
  console.log(`[deploy:data] targeted KRIC retry units=${units.length}, concurrency=${REPAIR_CONCURRENCY}`);
  ...
  db.transaction(() => {
    for (const group of groups.values()) applyGroup(db, group);
    metadata(db, "kric_timetable_station_failures", remaining.length);
    metadata(db, "kric_timetable_deploy_retry_units", units.length);
    metadata(db, "kric_timetable_deploy_repaired_units", units.length - remaining.length);
    metadata(db, "kric_timetable_deploy_repaired_at", new Date().toISOString());
  })();
  deriveRideEdges(db);
  ...
  if (remaining.length) {
    ...
    console.warn(`[deploy:data] targeted KRIC retry still unresolved=${remaining.length}`);
    process.exitCode = 2;
  } else {
    console.log(`[deploy:data] targeted KRIC retry repaired=${units.length}`);
  }
} finally {
  db.close();
}
