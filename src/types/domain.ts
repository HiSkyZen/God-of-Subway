...
export const LINE_NAMES: readonly LineName[] = ["1호선", "2호선", "3호선", "4호선", "5호선", "6호선", "7호선", "8호선", "9호선", ...EXTRA_LINES];
...
export interface PositionRow { subwayId?: string; subwayNm?: string; trainNo?: string; btrainNo?: string; statnNm?: string; statnTnm?: string; updnLine?: string; trainSttus?: string; recptnDt?: string; lastRecptnDt?: string; [key: string]: unknown; }
...
export interface Candidate { line: string; from: string; to: string; train_no: string; continuation_train_no: string; physical_continuation: boolean; service: string; direction: string; origin: string; destination: string; board_dt: Date; alight_dt: Date; wait_seconds: number; ride_seconds: number; delay_seconds: number; current_station: string; status: string; location_kind: "live" | "expected"; location_label: string; confidence: "높음" | "중간" | "낮음"; method: string; projected: boolean; [key: string]: unknown; }
export interface TransferInfo { station: string; seconds: number; base_seconds?: number; distance_m: number | null; alight_position: string; board_position: string; from_direction: string; to_direction: string; matched: string; mode?: "walk" | "same-platform" | "cross-platform" | "passage" | "branch" | "estimated"; source?: string; note?: string; crowding_multiplier?: number; crowding_level?: string; predicted_load?: number; }
...
export interface EngineData {
  s1: { weekday: Record<string, RawTrain>; saturday: Record<string, RawTrain>; holiday: Record<string, RawTrain>; };
  s1Stations: string[];
...
export interface PublicCandidate { train_no: string; continuation_train_no: string; physical_continuation: boolean; service: string; direction: string; origin: string; destination: string; board_dt: string | null; alight_dt: string | null; wait_seconds: number; ride_seconds: number; delay_seconds: number; current_station: string; status: string; location_kind: string; location_label: string; confidence: string; method: string; projected: boolean; live_detected: boolean; selected: boolean; }
