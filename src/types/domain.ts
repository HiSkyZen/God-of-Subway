...
export type LineName =
  ...
  | "경의중앙선" | "수인분당선" | "경춘선" | "경강선" | "서해선" | "공항철도";

export const EXTRA_LINES: readonly LineName[] = ["경의중앙선", "수인분당선", "경춘선", "경강선", "서해선", "공항철도"];
...
export interface Train {
  ...
  dest: string;
  stops: Stop[];
  linked_train_no?: string;
  continuation_train_no?: string;
  train_numbers?: string[];
  continuation_station?: string;
  continuation_gap_seconds?: number;
  continuation_start_sec?: number;
  physical_continuation?: boolean;
...
export interface Diagnostics {
  positions: number;
  matched: number;
  unmatched_train: string[];
  unmatched_station: string[];
...
export interface CalculateRoutePayload {
  ...
  baseline_minutes?: number | string | null;
  [key: string]: unknown;
}
...
export interface LiveTripPayload {
  segments: SegmentInput[];
  active_index?: number;
...
export interface EngineData {
  s1: {
    weekday: Record<string, RawTrain>;
    holiday: Record<string, RawTrain>;
  };
  s1Stations: string[];
  official: {
    meta?: { source?: string; version?: string; week_tags?: string[]; note?: string; [key: string]: unknown };
    days?: Record<string, Record<string, Record<string, RawMetroTrain>>>;
...
export interface RawTrain {
  ...
  dest?: string;
  linked_train_no?: string;
  stops?: Array<{ station?: string; arr?: number | null; dep?: number | null; call?: boolean }>;
  [key: string]: unknown;
}
...
export interface PublicCandidate {
  ...
  origin: string;
  destination: string;
  board_dt: string | null;
  alight_dt: string | null;
  wait_seconds: number;
  ride_seconds: number;
  delay_seconds: number;
  current_station: string;
