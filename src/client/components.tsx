...
function delayLabel(value?: number): string { const seconds = Number(value) || 0; return Math.abs(seconds) < 30 ? "정시권" : `${seconds >= 0 ? "+" : "−"}${Math.round(Math.abs(seconds) / 60)}분`; }
...
export function LivePanel({ trip, result, alertActive, arrivalAlertCapable, onAlert, onClearAlert, onFinishTransfer, onAlight, onStop }: { trip: LiveTripState; result: RouteResponse | null; alertActive: boolean; arrivalAlertCapable: boolean | null; onAlert: () => void; onClearAlert: () => void; onFinishTransfer: () => void; onAlight: () => void; onStop: () => void }): ReactElement {
  ...
  const title = trip.phase === "ride" ? `${trip.boardedTrainNo}열차 탑승 중` : trip.phase === "transfer" ? "환승 중" : trip.phase === "waiting" ? "다음 열차 탑승 대기" : "목적지 도착";
  const remaining = typeof result?.current_segment_remaining_seconds === "number" ? result.current_segment_remaining_seconds : 0;
  return <section className="live-panel"><div><span className="live-dot" /><strong>{title}</strong><p>{current?.current_station_name || current?.location || current?.from || "현재 위치 확인 중"} · 20초마다 최신 운행정보로 자동 재계산합니다.</p></div><div className="live-stats"><span>현재 열차 <b>{trip.boardedTrainNo || (trip.phase === "waiting" ? "선택 대기" : "환승")}</b></span><span>현재 위치 <b>{current?.current_station_name || current?.location || "-"}</b></span><span>{trip.phase === "transfer" ? "환승까지" : trip.phase === "waiting" ? "다음 열차" : trip.activeIndex >= trip.segments.length - 1 ? "목적지 하차까지" : "다음 환승까지"} <b>{trip.phase === "transfer" ? formatDuration(transferRemaining) : trip.phase === "waiting" ? formatClock(current?.board_dt) : formatDuration(remaining)}</b></span></div><div className="live-actions">{trip.phase === "transfer" && <button type="button" onClick={onFinishTransfer}>환승을 마쳤어요</button>}{trip.phase === "waiting" && <span>위 경로에서 실제 탑승할 열차를 선택하세요.</span>}{arrivalAlertCapable === false ? <span className="alert-unavailable">이 배포 환경에서는 백그라운드 도착 알림을 지원하지 않습니다.</span> : trip.phase === "ride" || alertActive ? <button type="button" className={alertActive ? "alert-active" : ""} onClick={alertActive ? onClearAlert : onAlert}>{alertActive ? "도착 알림 해제" : "도착 알림 설정"}</button> : null}{trip.phase === "ride" && <button type="button" onClick={onAlight}>이제 내렸어요</button>}{trip.phase === "done" && <button type="button" onClick={onStop}>여정 종료</button>}</div></section>;
}
