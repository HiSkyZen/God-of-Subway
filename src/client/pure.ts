...
export function experimentQuality(segments: RouteSegment[]): Confidence { const rank: Record<string, number> = { 높음: 3, 중간: 2, 낮음: 1 }; return (segments || []).reduce<Confidence>((quality, segment) => { const current = String(segment.confidence || "낮음"); return (rank[current] || 0) < (rank[String(quality)] || 0) ? current : quality; }, "높음"); }
export interface ExperimentMetricSummary { initial_error_min: number | null; final_error_min: number | null; baseline_error_min: number | null; error_gain_min: number | null; error_reduction_pct: number | null; first_platform_wait_min: number | null; recommendation_match_rate: number | null; }
export function experimentMetrics(experiment: ExperimentRecord): ExperimentMetricSummary { if (!experiment.completed_at || !experiment.actual_arrival) return { initial_error_min: null, final_error_min: null, baseline_error_min: null, error_gain_min: null, error_reduction_pct: null, first_platform_wait_min: null, recommendation_match_rate: null }; const absolute = (value: number | null): number | null => value == null ? null : Math.abs(value); const initialError = absolute(minutesDiff(experiment.actual_arrival, experiment.initial_eta)); const finalEta = experiment.final_eta ?? experiment.last_eta; const finalError = absolute(minutesDiff(experiment.actual_arrival, finalEta)); const baselineError = absolute(minutesDiff(experiment.actual_arrival, experiment.baseline_arrival)); const firstBoard = experiment.board_events.slice().sort((a, b) => a.at.localeCompare(b.at))[0]; const wait = firstBoard ? Math.max(0, minutesDiff(firstBoard.at, experiment.planned_platform_arrival) ?? 0) : null; const recommendations = experiment.board_events.filter((event) => Boolean(event.recommended_train_no)); const matchRate = recommendations.length ? recommendations.filter((event) => event.matches_recommendation).length / recommendations.length : null; const gain = baselineError != null && initialError != null ? baselineError - initialError : null; return { initial_error_min: initialError, final_error_min: finalError, baseline_error_min: baselineError, error_gain_min: gain, error_reduction_pct: gain != null && baselineError != null && baselineError > 0 ? gain / baselineError * 100 : null, first_platform_wait_min: wait, recommendation_match_rate: matchRate }; }
export function csvCell(value: unknown): string { const text = String(value ?? ""); return /[",\r\n]/.test(text) ? `"${text.replace(/"/g, '""')}"` : text; }
export function experimentsToJson(experiments: ExperimentRecord[]): string { return JSON.stringify(experiments, null, 2); }
export function experimentsToCsv(experiments: ExperimentRecord[]): string { const head = ["experiment_id", "created_at", "completed_at", "from", "to", "route", "transfer_count", "initial_eta", "actual_arrival", "initial_quality", "baseline_minutes"].join(","); const rows = experiments.map((experiment) => [experiment.id, experiment.created_at, experiment.completed_at, experiment.from, experiment.to, experiment.route_text, experiment.transfer_count, experiment.initial_eta, experiment.actual_arrival, experiment.initial_quality, experiment.baseline_minutes].map(csvCell).join(",")); return [head, ...rows].join("\r\n"); }
