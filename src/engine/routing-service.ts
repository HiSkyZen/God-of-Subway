...
export function transferSeconds(station: string, fromLine: string, toLine: string): number {
  const p = transferPairInfo(station, fromLine, toLine);
  return p ? Number(p.default_seconds ?? p.distance_seconds ?? DEFAULT_TRANSFER_SECONDS) : DEFAULT_TRANSFER_SECONDS;
}
...
function addEdge(adj: Map<string, AdjEdge[]>, from: Node, to: Node, weight: number, kind: PathEdge["kind"]): void {
  ...
  list.push({ to, weight: Math.trunc(weight), kind });
  adj.set(key, list);
}
...
export function routeAdjacency(mode: string): Map<string, AdjEdge[]> {
  ...
  for (const [line, names] of Object.entries(ROUTING_STATIONS_BY_LINE)) {
    for (const st of names) {
      const c = canonStation(st);
      const lines = stationLines.get(c) ?? [];
      if (!lines.includes(line)) lines.push(line);
      stationLines.set(c, lines);
    }
  }
  for (const [station, lines] of stationLines) {
    if (transferExclude().has(station) || lines.length < 2) continue;
    for (const a of lines) for (const b of lines) if (a !== b) addEdge(adj, [a, station], [b, station], transferSeconds(station, a, b), "transfer");
  }
...
function dijkstra(
  ...
  const source: Node = ["__SOURCE__", canonStation(start)];
  const target: Node = ["__TARGET__", canonStation(end)];
  const starts = stationLines(start).filter((line) => !excluded.has(line)).map((line) => [line, canonStation(start)] as Node);
  const targets = new Set(stationLines(end).filter((line) => !excluded.has(line)).map((line) => nodeKey([line, canonStation(end)])));
...
export function autoFindPath(start: string, end: string, mode: string, options?: RouteSearchOptions): Path {
  const from = canonStation(start);
  const to = canonStation(end);
  if (!from || !to) throw new Error("출발역과 도착역을 입력하세요.");
  if (from === to) throw new Error("출발역과 도착역이 같습니다.");
  if (!stationLines(from).length) throw new Error(`지원 노선에서 출발역 '${from}'을 찾지 못했습니다.`);
  if (!stationLines(to).length) throw new Error(`지원 노선에서 도착역 '${to}'을 찾지 못했습니다.`);
  const result = dijkstra(from, to, mode, undefined, new Set(), new Set(), options);
  if (!result) throw new Error(`${from} → ${to} 경로를 찾지 못했습니다.`);
  return { start: from, end: to, seconds: result.cost, edges: result.edges };
}
  ...
  while (accepted.length < maxRaw) {
    ...
    for (let i = 0; i < previous.nodes.length - 1; i += 1) {
      ...
      for (const path of accepted) {
        if (path.nodes.length > i && path.nodes.slice(0, i + 1).every((x, j) => nodeKey(x) === nodeKey(rootNodes[j])) && i < path.nodes.length - 1) {
          removedEdges.add(`${nodeKey(path.nodes[i])}|${nodeKey(path.nodes[i + 1])}`);
        }
      }
...
export function autoCandidateRoutes(start: string, end: string, mode: string, maxUnique = 8, options?: RouteSearchOptions): Array<{ path: Path; segments: SegmentInput[]; signature: string }> {
  ...
  for (const raw of yenPaths(start, end, mode, 36, options)) {
    const path: Path = { start: canonStation(start), end: canonStation(end), seconds: raw.cost, edges: raw.edges.filter((e) => e.kind !== "start" && e.kind !== "end") };
    try {
      ...
      seen.add(signature);
      result.push({ path, segments, signature });
      if (result.length >= maxUnique) break;
    } catch { /* Skip topology that cannot be covered by a train service. */ }
  }
  if (!result.length) {
    const path = autoFindPath(start, end, mode, options);
    const segments = enrichTransferSegments(autoPathToSegments(path, mode), mode);
    result.push({ path, segments, signature: segments.map((s) => `${s.line}:${s.from}:${s.to}`).join("|") });
  }
  return result;
}
...
export function autoPathToSegments(path: Path, mode: string): SegmentInput[] {
  ...
  for (const edge of path.edges) {
    ...
    if (current.line === line) {
      ...
      else { finish(1); current = { line, from, to, transfer_walk: 0 }; }
    } else {
      finish(DEFAULT_TRANSFER_SECONDS / 60);
      current = { line, from, to, transfer_walk: 0 };
    }
  }
  finish(0);
...
function directionStation(line: string, mode: string, start: string, end: string): string {
  ...
  for (const train of routeTrains(line, mode, from, to)) {
    const pair = routePair(train.stops, from, to);
    if (!pair) continue;
    for (let i = pair[1] + 1; i < train.stops.length; i += 1) {
      const station = canonStation(train.stops[i].station);
      if (station && station !== to) return station;
    }
  }
  return "";
}
...
function outgoingDirectionStation(line: string, mode: string, start: string, end: string): string {
  ...
  for (const train of routeTrains(line, mode, from, to)) {
    ...
    for (let i = pair[0] + 1; i < train.stops.length; i += 1) {
      const station = canonStation(train.stops[i].station);
      if (station && station !== from) return station;
    }
  }
  return "";
}
...
export function bestTransferDetail(station: string, from: SegmentInput, to: SegmentInput, mode: string): TransferInfo {
  ...
  if (!p) return { station: canonStation(station), seconds: DEFAULT_TRANSFER_SECONDS, distance_m: null, alight_position: "", board_position: "", from_direction: "", to_direction: "", matched: "fallback" };
  ...
  return {
    station: canonStation(station),
    seconds,
    distance_m: typeof p.distance_m === "number" ? p.distance_m : null,
    alight_position: pos(chosen.alight_car, chosen.alight_door),
    board_position: pos(chosen.board_car, chosen.board_door),
    from_direction: String(chosen.from_direction || incoming),
    to_direction: String(chosen.to_direction || outgoing),
    matched,
  };
}
...
export function enrichTransferSegments(segments: SegmentInput[], mode: string): SegmentInput[] {
  for (let i = 0; i < segments.length - 1; i += 1) {
    ...
    const info = bestTransferDetail(segments[i].to, segments[i], segments[i + 1], mode);
    segments[i].transfer_info = info;
    segments[i].transfer_seconds = info.seconds;
    segments[i].transfer_walk = Math.round(info.seconds / 60 * 1000) / 1000;
  }
  if (segments.length) {
    const last = segments[segments.length - 1];
    last.transfer_seconds = 0;
    last.transfer_walk = 0;
    last.transfer_info = null;
  }
  return segments;
}
