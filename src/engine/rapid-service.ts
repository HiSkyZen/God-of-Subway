...
/** Active 수도권 전철 lines with scheduled rapid/express service. AREX 직통열차는 일반 여객열차로 보고 후보군에서 제외한다. */
export const RAPID_SERVICE_LINES = new Set(["1호선", "4호선", "9호선", "경의중앙선", "수인분당선", "경춘선"]);
export type RailServiceKind = "rapid" | "local" | "unknown";
const RAPID_TEXT = /(급행|특급|express|rapid)/i;
...
export function servicePriority(service: unknown): number { return /급행|특급|express|rapid/i.test(String(service ?? "")) ? 20 : 10; }
