...
환승시간은 `data/transfer_data.json`의 측정/기존 값을 우선하며, 실제 승강장 구조와 충돌하면 `src/engine/transfer-policy.ts`의 검증 오버라이드가 우선합니다. 원본에서 소요시간이 비어 있어도 런타임은 거리 기반 모델 또는 180초 fallback으로 값을 만들어 **경로 계산 중 null 환승시간을 남기지 않습니다**. 다만 “값이 존재한다”와 “현장 실측이 완료됐다”는 구분하며, 업스트림 V13.5.4 감사 기준 124개 방향/위치 항목은 추가 검증 대상입니다.

자세한 우선순위와 공용선로 예외는 [`docs/TRANSFERS.md`](docs/TRANSFERS.md)를 참고하십시오.
...
원본 코드 및 데이터의 라이선스와 각 공공데이터 출처 조건을 따릅니다. 외부 자료는 구조 검증과 데이터 감사에 사용하며, 출처와 추정 여부를 문서에 남깁니다.
